...
docker build -t  ${IMAGE_NAME}:${SHORT_COMMIT} --build-arg WORD_USER=${WORD_USER} --build-arg WORD_EMAIL=${WORD_EMAIL} --build-arg WORD_PASS=${WORD_PASS} --build-arg PackageName=${PACKAGE_NAME} --build-arg PackageVersion=${PACKAGE_VERSION} --build-arg NexusUser=${NEXUS_USER} --build-arg NexusPassword=${NEXUS_USER_PWD} .
sleep 3
docker image ls 
