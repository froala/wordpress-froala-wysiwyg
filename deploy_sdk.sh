...
function generate_container_name(){
    ...
    if [ -z "${DEPL}" ]; then
        ...
        OLDEST_DATABASE_CONTAINER="${DB_SERVICE_NAME}-${AO_IDENTIFIER}-${CT_LOWER_INDEX}"
        CONTAINER_NAME="${LW_REPO_NAME}-${AO_IDENTIFIER}-${CT_INDEX}"
...
function deploy(){
    ...
    ssh -o "StrictHostKeyChecking no" -i  /tmp/sshkey.pem ${SSH_USER}@${DEPLOYMENT_SERVER} " sudo docker exec ${CONTAINER_NAME} mkdir -p /var/www/html/wp-content/plugins/froala && sleep 5" 
