...
ssh -o "StrictHostKeyChecking no" -i  /tmp/sshkey.pem ${SSH_USER}@${DEPLOYMENT_SERVER} " sudo docker exec ${CONTAINER_NAME} wget https://raw.githubusercontent.com/wp-cli/builds/gh-pages/phar/wp-cli.phar && chmod +x wp-cli.phar && cp -p wp-cli.phar /usr/local/bin/wp && cd /var/www/html/ &&rm -rf /var/www/html/wp-content/plugins/froala/admin/css && sleep 5" 
